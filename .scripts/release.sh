...
RELEASE_NOTES=$(claude -p --system-prompt "You are a release note generator for a CNC controller app. Write notes for end users, not developers. Only use the commit messages provided. Never invent changes. Skip internal-only changes like test fixes, refactors, and CI updates." "$PROMPT")
CLAUDE_EXIT_CODE=$?
