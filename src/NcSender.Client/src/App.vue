...
<template>
    ...
    <div class="settings-container">
      ...
      <div class="tab-content">
        ...
        <div v-if="activeTab === 'general'" class="tab-panel tab-panel--general">
            ...
            <div class="settings-group">
              ...
              <div class="setting-item setting-item--with-note">
                <div class="setting-item-content">
                  ...
                  <div class="settings-note">
                    Machine-coordinate Z position used for safe height moves (e.g. before tracing, go-to-zero, move spindle). Decrease if your machine triggers limits at Z0. Allowed range: {{ safeZMin }} to 0 {{ unitsPreference === 'imperial' ? 'in' : 'mm' }}.
                  </div>
                </div>
              ...
              <div class="setting-item">
                ...
                <div class="color-controls">
                  <div class="color-picker-container">
                    <ColorPicker v-model="accentColor" @change="saveColors" />
                    <ColorPicker v-model="gradientColor" @change="saveColors" />
                  </div>
          ...
          <div v-if="isKiosk" class="settings-section">
            <h3 class="section-title">Screen Orientation</h3>
            <div class="settings-note" style="margin-bottom: var(--gap-sm);">Rotate the host display for portrait or inverted mounting. This only affects the kiosk screen, not remote clients.</div>
            <div class="rotation-options">
              <button
                ...
                :key="opt.value"
                :class="['rotation-card', { active: selectedRotation === opt.value }]"
                @click="selectedRotation = opt.value"
              >
                <svg class="rotation-icon" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <template v-if="opt.value === 'normal'">
                    <rect x="4" y="6" width="40" height="28" rx="2" stroke="currentColor" stroke-width="2" />
                    <line x1="24" y1="34" x2="24" y2="40" stroke="currentColor" stroke-width="2" />
                    <line x1="16" y1="40" x2="32" y2="40" stroke="currentColor" stroke-width="2" stroke-linecap="round" />
                  </template>
                  <template v-if="opt.value === 'right'">
                    <rect x="10" y="4" width="28" height="40" rx="2" stroke="currentColor" stroke-width="2" />
                    <line x1="38" y1="24" x2="44" y2="24" stroke="currentColor" stroke-width="2" />
                    <line x1="44" y1="16" x2="44" y2="32" stroke="currentColor" stroke-width="2" stroke-linecap="round" />
                  </template>
                  <template v-if="opt.value === 'left'">
                    <rect x="10" y="4" width="28" height="40" rx="2" stroke="currentColor" stroke-width="2" />
                    <line x1="10" y1="24" x2="4" y2="24" stroke="currentColor" stroke-width="2" />
                    <line x1="4" y1="16" x2="4" y2="32" stroke="currentColor" stroke-width="2" stroke-linecap="round" />
                  </template>
                  <template v-if="opt.value === 'inverted'">
                    <rect x="4" y="14" width="40" height="28" rx="2" stroke="currentColor" stroke-width="2" />
                    <line x1="24" y1="14" x2="24" y2="8" stroke="currentColor" stroke-width="2" />
                    <line x1="16" y1="8" x2="32" y2="8" stroke="currentColor" stroke-width="2" stroke-linecap="round" />
                  </template>
                </svg>
                <span class="rotation-label">{{ opt.label }}</span>
              </button>
            </div>
            <div class="rotation-actions">
              <button
                class="units-button active"
                :disabled="rotationApplying"
                @click="applyRotation"
              >
                {{ rotationApplying ? 'Applying...' : 'Apply' }}
              </button>
            </div>
            <div v-if="rotationStatus" class="settings-note" style="margin-top: var(--gap-xs);">
              {{ rotationStatus }}
            </div>
          </div>
        ...
        <!-- Config Tab (FluidNC) -->
        <div v-if="activeTab === 'config'" class="tab-panel tab-panel--config">
          <ConfigTab />
        </div>
  ...
  <!-- Unhomed action warning migrated to server-owned GateDialog
       (POST /api/gate/ensure-homed). Rendered by GateHost with the same
       Continue/Abort buttons; visible on every client (browser + pendant). -->
  ...
  <!-- Gate Dialog Host (server-owned blocking prompts) -->
  <GateHost />
...
import { installGateDialogListeners } from './composables/useGateDialog';
...
const settingsTabs = computed(() =>
  allSettingsTabs.filter(t => {
    if (t.id === 'firmware') return serverState.displayFirmware;
    if (t.id === 'config') return serverState.displayConfig;
    return true;
  })
);
...
async function applyRotation() {
  rotationApplying.value = true;
  rotationStatus.value = '';
  try {
    const response = await fetch(`${api.baseUrl}/api/rotation`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ rotation: selectedRotation.value })
    });
    if (!response.ok) {
      const err = await response.json().catch(() => ({ error: 'Failed to set rotation' }));
      throw new Error(err.error || 'Failed to set rotation');
    }
    const result = await response.json();
    screenRotation.value = result.rotation;
  } catch (err: any) {
    rotationStatus.value = `Error: ${err.message}`;
  } finally {
    rotationApplying.value = false;
  }
}
...
onMounted(() => {
  ...
  if (settings?.pluginMessage?.modalPayload) {
    ...
    // Directly execute scripts from the persisted modal content.
    // ModalDialog's executeScripts() relies on v-html + Teleport + Transition
    // timing which is unreliable on restore. Extract and run the script here.
    const scriptMatch = settings.pluginMessage.modalPayload.content.match(/<script>([\s\S]*?)<\/script>/i);
...
const openSettings = async () => {
  ...
  try {
    ...
    const { loadSettings, getSettings } = await import('./lib/settings-store.js');
    await loadSettings();
...
const getAxisBitLabels = (setting: any): string[] => {
  ...
  const inferredAxisCount = axisCount.value > 0 ? axisCount.value : 3;
...
// Guard flag to prevent settings-changed broadcast from echoing a PATCH back
let _toolCountFromBroadcast = false;
...
const toggleRemoteControl = async (value: boolean) => {
  ...
  try {
    const { updateSettings } = await import('./lib/settings-store.js');
    await updateSettings({ remoteControl: { enabled: value } });
  } catch (error) {
...
.rotation-card.active .rotation-icon {
  color: var(--color-accent);
}
...
.rotation-card.active .rotation-label {
  color: var(--color-accent);
}
