...
const executeScripts = () => {
  ...
  // Delay script execution by one animation frame to ensure the DOM
  // (including Teleport targets) is fully rendered before scripts run.
  // Without this, document.getElementById() inside plugin scripts can
  // return null when restoring persisted modals on page refresh.
...
watch(() => props.content, () => {
  if (props.isOpen) {
    // Reset flag so scripts are re-executed when content changes
    // (e.g. server re-broadcasts dialog after WS reconnect)
    scriptsExecuted = false;
    nextTick(() => {
      nextTick(() => {
        if (!scriptsExecuted) {
