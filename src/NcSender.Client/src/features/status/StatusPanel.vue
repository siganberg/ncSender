...
<template>
  <section class="card status-card" :class="{ 'card-disabled': cardDisabled }">
    ...
    <div class="coords">
      <!-- Row 1: X | XY | Y (portrait: only these, landscape: all axes) -->
      <div class="axis-row axis-row--xy">
        <!-- X Card -->
        <div
        ...
        <!-- Z Card (in row 1 for landscape) -->
        <div
          :class="['axis-display', 'axis-z-landscape', { 'axis-disabled': axisControlsDisabled } ]"
      ...
      <!-- Row 2: Z (portrait only) -->
      <div class="axis-row axis-row--za">
        <!-- Z Card (portrait) -->
        <div
          :class="['axis-display', 'axis-z-portrait', { 'axis-disabled': axisControlsDisabled } ]"
          @mousedown="editingAxis !== 'z' && startLongPress('z', $event)"
          @mouseup="endLongPress('z', $event)"
          @mouseleave="cancelLongPress('z')"
          @touchstart="editingAxis !== 'z' && (($event.preventDefault()), startLongPress('z', $event))"
...
.axis-row > .axis-display:nth-child(4) {
...
@media (orientation: portrait) {
  /* Make coords container fill available space */
  .coords {
    flex: 1;
    display: flex;
    flex-direction: column;
  ...
  /* Row 1 only needs X, XY link, Y in portrait */
  .axis-row--xy {
