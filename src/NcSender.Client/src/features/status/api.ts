...
// Note: Work offsets are automatically refreshed server-side when G10 L2/L20 commands complete
...
export function zeroAxis(axis: 'X'|'Y'|'Z') {
...
export function setAxisValue(axis: 'X'|'Y'|'Z', value: number) {
  const a = String(axis).toUpperCase();
