...
const DEFAULT_TRAVEL_MM = 400;
const DEFAULT_TRAVEL_IN = 16;
...
function getAxisTravel(axis: 'X' | 'Y' | 'Z', isImperial: boolean): number {
  const store = useAppStore();
  const fallback = isImperial ? DEFAULT_TRAVEL_IN : DEFAULT_TRAVEL_MM;
  let raw: number | null = null;
  if (axis === 'X') raw = store.gridSizeX.value;
  else if (axis === 'Y') raw = store.gridSizeY.value;
  else if (axis === 'Z') raw = store.zMaxTravel.value;
  const valueMm = typeof raw === 'number' && Number.isFinite(raw) && raw > 0 ? raw : null;
  if (!valueMm) return fallback;
  return isImperial ? valueMm / 25.4 : valueMm;
}
...
export async function startContinuousJogSession(axis: 'X' | 'Y' | 'Z', direction: 1 | -1): Promise<ContinuousJogSession | null> {
  ...
  const feedRate = axis === 'Z' ? zFeedRate : xyFeedRate;
  const isImperial = unitCode === 'G20';
  const distance = getAxisTravel(axis, isImperial);
...
export async function startContinuousDiagonalJogSession(xDir: 1 | -1, yDir: 1 | -1): Promise<ContinuousJogSession | null> {
  ...
  const isImperial = unitCode === 'G20';
  const xDistance = getAxisTravel('X', isImperial) * xDir;
  const yDistance = getAxisTravel('Y', isImperial) * yDir;
  const command = `$J=${unitCode} G91 X${xDistance} Y${yDistance} F${xyFeedRate}`;
  const jogId = createJogId();
