...
class GCodeVisualizer {
    constructor() {
        ...
        // Execution distance tracking for progressive segment graying
        this.lineDistanceRange = new Map(); // lineNumber → { startDist, endDist }
        this.executionActive = false;

        // WCO (Work Coordinate Offset) from GRBL status
        // WCO = Workspace Offset + G92 Offset + Tool Length Offset
    ...
    // Set WCO (Work Coordinate Offset) from GRBL status
    // WCO = Workspace Offset + G92 Offset + Tool Length Offset
    setWCO(wco) {
        if (wco) {
    ...
    _axisOutOfBounds(x, y, z) {
        ...
        const zAbove = typeof maxZ === 'number' && z > maxZ + eps;
        const zOob = (typeof minZ === 'number' && z < minZ - eps) || zAbove;
    ...
    parseGCode(gcodeString) {
        ...
        const vertices = []; // Flat array of x,y,z coordinates (in machine coordinates)
        const colors = []; // Flat array of r,g,b values
        const opacities = []; // Per-vertex opacity
        const cumulativeDistances = []; // Per-vertex cumulative distance along toolpath
        const frames = []; // Line number to vertex index mapping (start vertex)
        let runningDistance = 0; // Cumulative path length

        let currentPos = { x: 0, y: 0, z: 0 }; // Work coordinates
        ...
        let currentSpindleSpeed = 0; // Track S-value
        let isIncremental = false; // Track G90/G91 mode
        ...
        // Snapshot WCO at render time for spindle position correction
        this.renderWCO = { x: this.wco.x, y: this.wco.y, z: this.wco.z };

        // Convert work coordinates to machine coordinates using WCO
        // machinePos = workPos + WCO (which includes workspace offset + G92 + TLO)
        const toMachineCoords = (workPos) => {
            return {
                x: workPos.x + this.wco.x,
                y: workPos.y + this.wco.y,
                z: workPos.z + this.wco.z
            };
        };
        ...
        lines.forEach((line, lineIndex) => {
            ...
            if (cleanLine.includes('G28')) {
                ...
                const machineStart = toMachineCoords(currentPos);
                const zMachineHome = this.gridBounds?.maxZ ?? 0;

                if (machineStart.z !== zMachineHome) {
                    ...
                    const g28Len = Math.abs(zMachineHome - machineStart.z);
                    const g28StartDist = runningDistance;
                    cumulativeDistances.push(runningDistance);
                    runningDistance += g28Len;
                    cumulativeDistances.push(runningDistance);
                    this.lineDistanceRange.set(lineNumber, { startDist: g28StartDist, endDist: runningDistance });

                    vertices.push(machineStart.x, machineStart.y, machineStart.z);
                    vertices.push(machineStart.x, machineStart.y, zMachineHome);
                    colors.push(color.r, color.g, color.b);
                    colors.push(color.r, color.g, color.b);
                    opacities.push(1.0, 1.0); // G28 is always full opacity (rapid move)
                }

                currentPos.z = zMachineHome - this.wco.z;
                return;
            }
            ...
            // Track spindle speed (S-value)
            if (sMatch) {
                currentSpindleSpeed = parseFloat(sMatch[1]);
            }
            ...
            if (gMatch) {
                gMatch.forEach(g => {
                    ...
                    // Track incremental/absolute mode
                    if (gCode === 90) {
                        isIncremental = false;
                    } else if (gCode === 91) {
                        isIncremental = true;
            ...
            if (isIncremental) {
                if (xMatch) newPos.x = currentPos.x + parseFloat(xMatch[1]) * unitScale;
                if (yMatch) newPos.y = currentPos.y + parseFloat(yMatch[1]) * unitScale;
                if (zMatch) newPos.z = currentPos.z + parseFloat(zMatch[1]) * unitScale;
            } else {
            ...
            if (hasMovement && lastMoveType !== null) {
                ...
                // Convert to machine coordinates for rendering and bounds checking
                const machineStart = toMachineCoords(currentPos);
                const machineEnd = toMachineCoords(newPos);

                // Check if move is out of bounds (using machine coordinates)
                const oob1 = this._axisOutOfBounds(machineStart.x, machineStart.y, machineStart.z);
                const oob2 = this._axisOutOfBounds(machineEnd.x, machineEnd.y, machineEnd.z);
                ...
                if (lastMoveType === 2 || lastMoveType === 3) {
                    ...
                    // Transform each point to machine coordinates using WCO
                    for (let idx = 0; idx < points.length - 1; idx++) {
                        const point1 = points[idx];
                        const point2 = points[idx + 1];
                        const z1 = currentPos.z + (newPos.z - currentPos.z) * (idx / (points.length - 1));
                        const z2 = currentPos.z + (newPos.z - currentPos.z) * ((idx + 1) / (points.length - 1));

                        const mx1 = point1.x + this.wco.x;
                        const my1 = point1.y + this.wco.y;
                        const mz1 = z1 + this.wco.z;
                        const mx2 = point2.x + this.wco.x;
                        const my2 = point2.y + this.wco.y;
                        const mz2 = z2 + this.wco.z;

                        // Cumulative distance for this sub-segment
                        const segLen = Math.sqrt((mx2 - mx1) ** 2 + (my2 - my1) ** 2 + (mz2 - mz1) ** 2);
                        cumulativeDistances.push(runningDistance);
                        runningDistance += segLen;
                        cumulativeDistances.push(runningDistance);

                        // Push as pair for LineSegments (in machine coordinates)
                        vertices.push(mx1, my1, mz1);
                        vertices.push(mx2, my2, mz2);
                        colors.push(color.r, color.g, color.b);
                        colors.push(color.r, color.g, color.b);
                        opacities.push(vertexOpacity, vertexOpacity);
                    }
                } else {
                    // Linear move - add as pair (in machine coordinates)
                    const segLen = Math.sqrt(
                        (machineEnd.x - machineStart.x) ** 2 +
                        (machineEnd.y - machineStart.y) ** 2 +
                        (machineEnd.z - machineStart.z) ** 2
                    );
                    cumulativeDistances.push(runningDistance);
                    runningDistance += segLen;
                    cumulativeDistances.push(runningDistance);
                    ...
                    vertices.push(machineStart.x, machineStart.y, machineStart.z);
                    vertices.push(machineEnd.x, machineEnd.y, machineEnd.z);
                ...
                this.lineDistanceRange.set(lineNumber, { startDist: lineStartDist, endDist: runningDistance });

                currentPos = newPos;
            }
        });

        return { vertices, colors, opacities, cumulativeDistances, frames, hasOutOfBounds };
    }
    ...
    render(gcodeString) {
        ...
        const { vertices, colors, opacities, cumulativeDistances, frames, hasOutOfBounds } = this.parseGCode(gcodeString);
        ...
        geometry.setAttribute('cumulativeDistance', new THREE.Float32BufferAttribute(cumulativeDistances, 1));
        ...
        const material = new THREE.ShaderMaterial({
            vertexShader: `
                ...
                attribute float cumulativeDistance;
                varying vec3 vColor;
                varying float vOpacity;
                varying float vCumulativeDistance;

                void main() {
                    vColor = color;
                    vOpacity = opacity;
                    vCumulativeDistance = cumulativeDistance;
            ...
            fragmentShader: `
                ...
                uniform float executionDistance;
                uniform bool executionActive;
                uniform vec3 completedCuttingColor;
                uniform vec3 completedRapidColor;
                ...
                void main() {
                    ...
                    // Progressive execution graying
                    if (executionActive && vCumulativeDistance >= 0.0 && vCumulativeDistance <= executionDistance) {
                        vec3 compColor = isRapid ? completedRapidColor : completedCuttingColor;
                        float finalOpacity = useVertexOpacity ? vOpacity : baseOpacity;
                        gl_FragColor = vec4(compColor, finalOpacity);
                        return;
                    }
            ...
            uniforms: {
                ...
                useVertexOpacity: { value: false },
                executionDistance: { value: -1.0 },
                executionActive: { value: false },
                completedCuttingColor: { value: new THREE.Color(this.moveColors.completedCutting) },
                completedRapidColor: { value: new THREE.Color(this.moveColors.completedRapid) }
            },
            transparent: true,
            depthTest: false
        });
    ...
    resetCompletedLines() {
        ...
        this.completedLines.forEach(lineNumber => {
            ...
            const toolNum = this.lineToolNumber.get(lineNumber);
            const isToolHidden = toolNum !== undefined && this.toolVisibility.get(toolNum) === false;
            const targetColor = isToolHidden ? hiddenColor : this.getLineColor(lineNumber);
        ...
        try {
            ...
            for (const [lineNumber, range] of this.lineNumberMap.entries()) {
                ...
                for (let i = startVertexIdx; i < endVertexIdx; i++) {
                    ...
                    let c;
                    if (isToolHidden) {
                        // Tool is hidden, use black color (discarded by shader)
                        c = hiddenColor;
                    } else if (isCompleted) {
    ...
    setExecutionDistance(distance) {
        this.pathLines.forEach(line => {
            if (line.material.uniforms && line.material.uniforms.executionDistance) {
                line.material.uniforms.executionDistance.value = distance;
            }
        });
    }

    setExecutionActive(active) {
        this.executionActive = active;
        this.pathLines.forEach(line => {
            if (line.material.uniforms && line.material.uniforms.executionActive) {
                line.material.uniforms.executionActive.value = active;
            }
        });
    }

    getLineEndDistance(lineNumber) {
        const range = this.lineDistanceRange.get(lineNumber);
        return range ? range.endDist : 0;
    }

    getExecutionDistanceForPosition(lineNumber, machinePos) {
        const line = this.pathLines[0];
        if (!line) return 0;
        ...
        let bestCumulativeDist = cumDist[startVertexIdx] || 0;

        // LineSegments uses pairs: (i, i+1), (i+2, i+3), etc.
        // Line uses consecutive: (i, i+1), (i+1, i+2), etc.
        const isLineSegments = line.isLineSegments;
        const step = isLineSegments ? 2 : 1;

        for (let i = startVertexIdx; i < endVertexIdx - 1; i += step) {
            const i2 = i + 1;
            ...
            if (dist < closestDist) {
                closestDist = dist;
                // Interpolate cumulative distance at projection point
                const d1 = cumDist[i];
                const d2 = cumDist[i2];
                bestCumulativeDist = d1 + t * (d2 - d1);
            }
        }

        return bestCumulativeDist;
    }
