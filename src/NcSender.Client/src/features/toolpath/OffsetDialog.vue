<!--
  ...
  Dialog for entering offset values for toolpath translation.
  - Top view: X/Y only
  - Front view: Z only
  - 3D view: X/Y/Z
-->
      ...
      <div class="dialog-content">
        <p class="dialog-description">
          <template v-if="is3dView">
            Enter offset values to translate the toolpath in all axes.
          </template>
          <template v-else-if="isTopView">
        ...
        <!-- X and Y inputs (Top and 3D view) -->
        <template v-if="isTopView || is3dView">
          <div class="input-row">
        ...
        <!-- Z input (Front and 3D view) -->
        <div v-if="isFrontView || is3dView" class="input-row">
...
const is3dView = computed(() => props.view === 'iso');
...
const isValid = computed(() => {
  if (is3dView.value) {
    // 3D view: any axis can be offset
    return (offsetX.value !== 0 || offsetY.value !== 0 || offsetZ.value !== 0) &&
           !isNaN(offsetX.value) && !isNaN(offsetY.value) && !isNaN(offsetZ.value);
  } else if (isTopView.value) {
...
watch(() => props.show, (newVal) => {
  if (newVal) {
    ...
    offsetZ.value = 0;
    nextTick(() => {
      if (isTopView.value || is3dView.value) {
        xInput.value?.focus();
        xInput.value?.select();
      } else {
        zInput.value?.focus();
