...
watch(activeTab, async (tab) => {
  if (tab === 'terminal') {
    ...
    if (autoScroll.value && scrollerRef.value) {
      scrollTerminalToBottom(scrollerRef.value);
    }
  }
});
...
const loadCommandHistory = async () => {
  ...
  try {
    // Use pre-loaded command history from /api/init if available
    const preloaded = getCommandHistoryFromInit();
    if (preloaded && preloaded.length > 0) {
      commandHistory.value = preloaded;
    } else {
...
function scrollTerminalToBottom(scroller: any) {
  if (!scroller) return;
  scroller.scrollToItem(terminalLines.value.length - 1);
  // scrollToItem aligns the item's top edge into view. For long wrapped lines
  // the bottom is clipped. Wait for the virtual scroller to render and measure
  // the item, then nudge the container to the very bottom.
...
onMounted(async () => {
  ...
  stopAutoScrollBindings = store.startAutoScrollBindings(async (evt) => {
    ...
    if (showTerminalModal.value && autoScrollTerminalModal.value && modalTerminalScrollerRef.value) {
      await nextTick();
      scrollTerminalToBottom(modalTerminalScrollerRef.value);
    }
  });
});
...
function closeGcodeModal() {
  ...
  hasUnsavedChanges.value = false;
  showGcodeModal.value = false;
  keyboardStore.clearMonacoInstance();
}
