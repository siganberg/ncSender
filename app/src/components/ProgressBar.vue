...
// Fake animation parameters
const totalSeconds = ref(180); // 3 minutes simulated total
const startAt = ref(Date.now());
const elapsedSec = ref(0);

const percent = computed(() => {
  const p = Math.min(100, (elapsedSec.value / totalSeconds.value) * 100);
  return Math.round(p * 10) / 10; // one decimal for smoothness
});
...
onMounted(() => {
  // Start a looped fake progress: advance, then reset smoothly
  const tick = () => {
    const now = Date.now();
    elapsedSec.value = Math.min(totalSeconds.value, (now - startAt.value) / 1000);
    if (elapsedSec.value >= totalSeconds.value) {
      // Reset with a small pause to simulate next job phase
      setTimeout(() => {
        startAt.value = Date.now();
        totalSeconds.value = 120 + Math.floor(Math.random() * 180); // 2–5 minutes next cycle
        elapsedSec.value = 0;
      }, 600);
    }
  };
